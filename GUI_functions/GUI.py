...
def makeHexagonsOnMode(window, window_size, positions):
    allShapes = []
    color = '#DCDCDC'

    for tuple_xy in positions: 
        if tuple_xy == (0,0): 
            color = '#FFD700'
        else: 
            color = '#DCDCDC'
...
def repeatTrials(gates, window, refresh_rate, mouse):
    ...
    while check == None:
        SSVEP_record = True
        for i in range(3):
            check = flashMouseResponse(gates, window, refresh_rate, mouse, text)
        SSVEP_record = False
        # check = into aidans functin
